...
int maiores_que_x(NoArv* a, int x);
...
int main(){
    ...
    int maiores = maiores_que_x(raiz,1);
...
int maiores_que_x(NoArv* a, int x) {
    if (a == NULL) {
        return 0; // A árvore está vazia, não há nós maiores que x
    } else {
        int count = 0;
        if (a->info > x) {
            // O nó atual armazena um valor maior que x, conte-o
            count++;
        }

        // Recursivamente conte os nós maiores que x nas subárvores esquerda e direita
        count += maiores_que_x(a->esq, x);
        count += maiores_que_x(a->dir, x);
